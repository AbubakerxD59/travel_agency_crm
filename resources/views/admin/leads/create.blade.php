        ...
        <div class="mt-8 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-8">
            <form id="lead-create-form" method="POST" action="{{ route('admin.leads.store') }}" class="space-y-4">
                ...
                @php
                    ...
                    $itineraryRows = old('itineraries', [[]]);
                    $passengerRows = old('passengers', [[]]);
                    $packageCostRows = old('package_costs', [[]]);
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <div class="min-w-0">
                        <label for="lead_agent_id" class="block text-sm font-medium text-concierge-navy">Agent <span
                                class="font-normal text-concierge-muted">(optional)</span></label>
                    ...
                    <div class="min-w-0">
                        ...
                        <input id="lead_order_type" name="order_type" type="text" required value="{{ old('order_type') }}"
                            placeholder="e.g. Umrah package" class="{{ $fieldClass }}">
                    </div>
                    <div class="min-w-0">
                        ...
                        <input id="lead_vendor_reference" name="vendor_reference" type="text" value="{{ old('vendor_reference') }}"
                            class="{{ $fieldClass }}">
                    </div>
                </div>
                    ...
                    <div class="min-w-0">
                        ...
                        <select id="lead_status" name="status" required class="{{ $fieldClass }}">
                            @foreach ($statuses as $value => $label)
                                <option value="{{ $value }}" @selected(old('status', \App\Models\Lead::STATUS_NEW) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="min-w-0">
                        ...
                        <select id="lead_destination_id" name="destination_id" required class="{{ $fieldClass }}">
                            <option value="" disabled @selected(! old('destination_id'))>Select destination</option>
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <div class="min-w-0">
                        ...
                        <input id="lead_travel_date" name="travel_date" type="date" required value="{{ old('travel_date') }}"
                            class="{{ $fieldClass }}">
                    </div>
                    <div class="min-w-0">
                        ...
                        <input id="lead_balance_due_date" name="balance_due_date" type="date" value="{{ old('balance_due_date') }}"
                            class="{{ $fieldClass }}">
                    </div>
                    <div class="min-w-0">
                        ...
                        <div class="mt-1.5 flex min-h-[2.625rem] flex-row flex-nowrap items-center gap-6 rounded-xl border border-slate-200/80 bg-slate-50/50 px-4 py-2.5">
                            <label class="flex cursor-pointer items-center gap-2 text-sm text-concierge-navy whitespace-nowrap">
                                <input type="checkbox" name="ziarat_makkah" value="1"
                                    ...
                                    @checked(old('ziarat_makkah'))>
                                Makkah
                            </label>
                            <label class="flex cursor-pointer items-center gap-2 text-sm text-concierge-navy whitespace-nowrap">
                                <input type="checkbox" name="ziarat_madinah" value="1"
                                    class="rounded border-slate-300 text-concierge-accent focus:ring-concierge-accent/30"
                                    @checked(old('ziarat_madinah'))>
                                Madinah
                            </label>
                        </div>
                    </div>
                ...
                <div class="min-w-0">
                    <label for="lead_flight_itinerary" class="block text-sm font-medium text-concierge-navy">Flight itinerary
                        <span class="font-normal text-concierge-muted">(optional)</span></label>
                    <textarea id="lead_flight_itinerary" name="flight_itinerary" rows="4" class="{{ $fieldClass }}">{{ old('flight_itinerary') }}</textarea>
                </div>
