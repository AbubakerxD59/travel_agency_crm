...
@section('content')
    <div id="js-agents-config" class="hidden"
         data-url-base="{{ route('admin.agents.index') }}"
         data-can-manage="{{ $canManageAgents ? '1' : '0' }}"
    ...
    <div class="mx-auto max-w-7xl">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-concierge-muted">Manage concierge agents and their accounts.</p>
            </div>
        ...
        <div class="agents-list-wrap mt-4 overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm sm:p-6">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($agents as $agent)
                            <tr class="hover:bg-slate-50/50"
                                ...
                                <td class="px-6 py-4">
                                    <span class="concierge-pill concierge-pill-contacted">{{ $agent->roles->first()?->name ?? 'agent' }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm text-concierge-muted">{{ $agent->created_at?->format('M j, Y') }}</td>
                                @if ($canManageAgents)
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex flex-wrap items-center justify-end gap-1">
                                            ...
                                            <button type="button"
                                                    ...
                                                    data-permissions-agent="{{ $agent->id }}"
                                                    title="Permissions">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H3.75v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
                                                </svg>
                                            </button>
                                            <button type="button"
                        ...
                        @empty
                            <tr class="agents-index-empty">
                                <td colspan="{{ $canManageAgents ? 6 : 5 }}" class="px-6 py-10 text-center text-sm text-concierge-muted">
            ...
            <p id="agents-filter-no-results" class="hidden px-6 py-8 text-center text-sm text-concierge-muted" role="status">
    ...
    {{-- Modal --}}
    <div id="agent-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 p-4" aria-hidden="true" role="dialog" aria-labelledby="agent-modal-title">
        <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                ...
                <button type="button" data-close-agent-modal class="rounded-lg p-2 text-concierge-muted hover:bg-slate-100 hover:text-concierge-navy" aria-label="Close">
            ...
            <form id="store-agent-form" method="POST" action="{{ route('admin.agents.store') }}" class="space-y-4 px-6 py-5">
                @csrf
                ...
                <div>
                    <label for="modal_phone_number" class="block text-sm font-medium text-concierge-navy">Phone number</label>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-concierge-muted">Super admins are not created from this form.</p>
                </div>
                ...
                <div>
                    ...
                    <div class="relative mt-1.5">
                        ...
                        <button type="button"
                                ...
                                data-password-toggle="modal_password"
                ...
                <div>
                    <label for="modal_confirm_password" class="block text-sm font-medium text-concierge-navy">Confirm password</label>
                    <div class="relative mt-1.5">
                        <input id="modal_confirm_password" name="confirm_password" type="password" required autocomplete="new-password"
                               class="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-2.5 pl-4 pr-11 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                        <button type="button"
                                class="absolute inset-y-0 right-0 flex cursor-pointer items-center rounded-r-xl px-3 text-concierge-muted hover:text-concierge-navy"
                                data-password-toggle="modal_confirm_password"
                                aria-label="Show password"
                                aria-pressed="false">
                            <span data-icon="show" class="flex">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
    ...
    @if ($canManageAgents)
        {{-- Edit agent modal --}}
        <div id="edit-agent-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 p-4" aria-hidden="true" role="dialog" aria-labelledby="edit-agent-modal-title">
            <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    <h2 id="edit-agent-modal-title" class="text-lg font-semibold text-concierge-navy">Edit agent</h2>
                    <button type="button" data-close-edit-agent-modal class="rounded-lg p-2 text-concierge-muted hover:bg-slate-100 hover:text-concierge-navy" aria-label="Close">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <form id="edit-agent-form" method="POST" class="space-y-4 px-6 py-5">
                    ...
                    <div>
                        <label for="edit_modal_name" class="block text-sm font-medium text-concierge-navy">Name</label>
                        <input id="edit_modal_name" name="name" type="text" required autocomplete="name"
                               class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-2.5 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                    </div>

                    <div>
                        <label for="edit_modal_email" class="block text-sm font-medium text-concierge-navy">Email</label>
                        <input id="edit_modal_email" name="email" type="email" required autocomplete="email"
                               class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-2.5 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                    </div>

                    <div>
                        <label for="edit_modal_phone_number" class="block text-sm font-medium text-concierge-navy">Phone number</label>
                        <input id="edit_modal_phone_number" name="phone_number" type="text" required autocomplete="tel"
                               class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-2.5 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                    </div>

                    <div>
                        <label for="edit_modal_password" class="block text-sm font-medium text-concierge-navy">New password <span class="font-normal text-concierge-muted">(optional)</span></label>
                        <div class="relative mt-1.5">
                            <input id="edit_modal_password" name="password" type="password" autocomplete="new-password"
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-2.5 pl-4 pr-11 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                            <button type="button"
                                    class="absolute inset-y-0 right-0 flex cursor-pointer items-center rounded-r-xl px-3 text-concierge-muted hover:text-concierge-navy"
                                    data-password-toggle="edit_modal_password"
                                    aria-label="Show password"
                                    aria-pressed="false">
                                <span data-icon="show" class="flex">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                    ...
                    <div>
                        <label for="edit_modal_confirm_password" class="block text-sm font-medium text-concierge-navy">Confirm new password</label>
                        <div class="relative mt-1.5">
                            <input id="edit_modal_confirm_password" name="confirm_password" type="password" autocomplete="new-password"
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-2.5 pl-4 pr-11 text-sm focus:border-concierge-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-concierge-accent/20">
                            <button type="button"
                                    class="absolute inset-y-0 right-0 flex cursor-pointer items-center rounded-r-xl px-3 text-concierge-muted hover:text-concierge-navy"
                                    data-password-toggle="edit_modal_confirm_password"
                                    aria-label="Show password"
                                    aria-pressed="false">
                                <span data-icon="show" class="flex">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
        ...
        {{-- Permissions modal --}}
        <div id="permissions-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 p-4" aria-hidden="true" role="dialog" aria-labelledby="permissions-modal-title">
            <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    ...
                    <button type="button" data-close-permissions-modal class="rounded-lg p-2 text-concierge-muted hover:bg-slate-100 hover:text-concierge-navy" aria-label="Close">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
